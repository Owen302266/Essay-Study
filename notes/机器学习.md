...
按照两种分布的混合分布的变量的分布更加随机，相较两种原始分布更难估计。
...
1. 初始化集群代表$\left\{\mu_{k}^{\text {old }}\right\}$
...
$$
GAN通过选择参数向量的可微函数$T(x)$来参数化鉴别器$T_\phi(x)$，这有效地减少了鉴别器的搜索空间，并为每个值定义了不同类型的散度。
可以得到GAN解决的极大极小问题：
$$
\min _{\theta} \max _{\varphi} \mathrm{E}_{\mathrm{x} \sim p_{\mathcal{D}}}\left[T_{\varphi}(\mathrm{x})\right]-\mathrm{E}_{\mathrm{x} \sim p(x \mid \theta)}\left[g\left(T_{\varphi}(\mathrm{x})\right)\right]
$$
根据选择的$g$函数的不同，分为不同类的GAN。

##### Application to likelihood-free models应用于无似然模型

模型分布可以写为：
$$
p(x \mid \theta)=\int \delta\left(x-G_{\theta}(z)\right) \mathcal{N}(z \mid 0, I) d z
$$
样本x被建模为生成函数$G_θ(z)$的输出，其输入$z$由i.i.d.给出，高斯变量。因此，该生成模型可以被解释为PPCA到非线性编码器$G_θ(z)$的推广。后者通常再次被建模为多层神经网络。

##### Likelihood ratio estimation viewpoint似然比估计观点

在一个足够一般的函数$T(x)$上求解问题，可以得到$\mathrm{KL}\left(p_{\mathcal{D}}(x) \| p(x \mid \theta)\right)$。
GAN可以被解释为执行数据和模型分布之间的似然比的估计，作为学习过程中的一个步骤。为了估计KL发散而估计似然比的想法在基于变分推理的其他学习问题中是有用的

### Undirected Generative Models无向生成模型

与有向生成模型不同，无向生成模型假设隐藏变量和观察变量的联合分布，捕捉两组变量之间的亲和关系，而不是因果关系。

#### Restricted Boltzmann Machines (RBM)受限玻尔兹曼机

##### Model

在RBM模型中，可见和潜在变量的联合分布是对数线性模型：
$$
p(x, z \mid \theta)=\frac{1}{Z(\theta)} \exp (-E(x, z \mid \theta))
$$
其中能量函数为：
$$
E(x, z \mid \theta)=-a^{T} z-b^{T} x-x^{T} W z
$$
配分函数为：
$$
 Z(\theta)=\sum_{x, z} \exp (-E(x, z \mid \theta))
$$
RBM模型只捕获了能量函数中交叉项的特性，因此是（限制R）模型。
根据该模型，当分别以所有观察或隐藏变量为条件时，很容易计算每个隐藏或观察变量的分布。
$z$中的变量在以$x$为条件时是相互独立的，并且类似地，$x$中的变量在给定$z$时是独立的，条件分布为：
$$
\begin{array}{l}
p\left(z_{j}=1 \mid x, \theta\right)=\sigma\left(w_{j}^{T} x+a_{j}\right) \\
p\left(x_{i}=1 \mid z, \theta\right)=\sigma\left(\tilde{w}_{i}^{T} z+b_{i}\right)
\end{array}
$$
...
当前时刻梯度可以计算为：
$$
\left.\nabla_{w_{i j}} \ln p(x \mid \theta)\right|_{\theta=\theta^{\text {old }}}=\mathrm{E}_{\mathrm{z}_{j} \sim p\left(z_{j} \mid x, \theta^{\text {old }}\right)}\left[x_{i} \mathrm{z}_{j}\right]-\mathrm{E}_{\mathrm{x}_{i}, \mathrm{z}_{j} \sim p\left(x, z_{j} \mid \theta^{\text {old }}\right)}\left[\mathrm{x}_{i} \mathrm{z}_{j}\right]
$$
使用条件分布公式进行简化：
$$
\begin{array}{c}
\left.\nabla_{a_{j}} \ln p(x \mid \theta)\right|_{\theta=\theta^{\text {old }}}=\mathrm{E}_{\mathrm{z}_{j} \sim p\left(z_{j} \mid x, \theta^{\text {old }}\right)}\left[\mathrm{z}_{j}\right]-\mathrm{E}_{\mathrm{z}_{j} \sim p\left(z_{j} \mid \theta^{\text {old }}\right)}\left[\mathrm{z}_{j}\right] \\
\left.\nabla_{b_{i}} \ln p(x \mid \theta)\right|_{\theta=\theta^{\text {old }}}=x_{i}-\mathrm{E}_{\mathrm{x}_{i} \sim p\left(x_{i} \mid \theta^{\text {old }}\right)}\left[\mathrm{x}_{i}\right]
\end{array}
$$
为了评估梯度负分量的期望值，通常使用第8章讨论的马尔可夫链MC（MCMC）技术，对比发散（CD）的简化方法是有效的近似。条件概率公式所得样本用于估计预期值：
$$
\left.\nabla_{w_{i j}} \ln p(x \mid \theta)\right|_{\theta=\theta^{\text {old }}} \simeq x_{i} z_{j}^{(0)}-x_{i}^{(1)} z_{j}^{(1)}
$$
具有多层隐变量的RBM的推广被称为深玻尔兹曼机。

### Discriminative Models判别模型

当我们的目标是学习数据$x$的表示$z$，我们可以定义联合分布：
$$
p(x, z \mid \theta)=p_{\mathcal{D}}(x) p(z \mid x, \theta)
$$
产生的信息最大化问题表示为:
$$
\max _{\theta} I_{p_{\mathcal{D}}(x) p(z \mid x, \theta)}(\mathrm{x} ; \mathrm{z})
$$
一个典型的解决方案是采用MM方法，它类似于上述ML学习的EM算法，引入变分分布$q(x|z)$，并且互信息的下界为：
$$
I_{p_{\mathcal{D}}(x) p(z \mid x, \theta)}(\mathrm{x} ; \mathrm{z}) \geq H\left(p_{\mathcal{D}}(x)\right)+\mathrm{E}_{\mathrm{x}, \mathrm{z} \sim p_{\mathcal{D}}(x) p(z \mid x, \theta)}[\ln q(\mathrm{x} \mid \mathrm{z})]
$$
当变分分布与后验相同时，互信息的下界是最紧密的：
$$
p(x \mid z, \theta)=p_{\mathcal{D}}(x) p(z \mid x, \theta) /\left(\sum_{x} p_{\mathcal{D}}(x) p(z \mid x, \theta)\right)
$$

##### 基于推理的解释

互信息可以与给定表示z时推断x的错误概率相关。
通过在$θ$上最大化互信息的下界，模型$p(z| x，θ)$获得表示$z$，使得预测器$q(x| z)$确保了在给定$z$的情况下对$x$的良好重构。

##### 信息瓶颈法

InfoMax原理的一个重要变化产生了信息瓶颈方法。
假设存在与$x$联合分布的附加变量$y$，其代表期望的信息，但未被观察到。
通过最大化互信息$I(y; z)$在目标未观测变量$y$和表示$z$之间的关系，其中存在旨在最小化表示的复杂度的正则化项。

### Autoencoders自编码器

自动编码器包括编码器和解码器的参数模型：
$$
z=F_{\theta}(x), x=G_{\theta}(z)
$$
学习通常是按照ERM问题来表述的:
$$
\min _{\theta} \sum_{n=1}^{N} \ell\left(x_{n}, G_{\theta}\left(F_{\theta}\left(x_{n}\right)\right)\right)
$$

#### PCA

PCA假设编码器和解码器是线性的，并且通过转置将它们的权重矩阵联系起来：
$$
F_{\theta}(x)=W^{T} x , G_{\theta}(z)=W z
$$
使用二次损失函数，学习问题可表示为：
$$
\min _{W} \sum_{n=1}^{N}\left\|x_{n}-W W^{T} x_{n}\right\|^{2}
$$
主成分分析可以被视为一种特殊情况的PPCA。

#### Dictionary learning

在字典学习中，解码器是线性的。
编码器仅受可行潜在变量$z$的集合上的约束的限制。一个典型的约束是稀疏性：本征向量应该具有少量的非零元素。将$z$的可行值的集合定义为$C$，字典学习问题可以被公式化为：
$$
\min _{W,\left\{z_{n}\right\} \in \mathcal{C}} \frac{1}{N} \sum_{n=1}^{N}\left\|x_{n}-W z_{n}\right\|^{2}
$$
矩阵W可以被认为是用于描述数据的$M$个特征（其列）的字典，可以采用交叉优化的方法：
1. 对于一组固定的特征向量$\{z_n\}$，在$W$上进行优化，这是标准的最小二乘问题
2. 对于固定的$W$在每个本征向量$\{z_n\}$上进行优化。第二个问题可以通过使用标准的基于稀疏性的方法来解决，如LASSO

#### Multi-layer autoencoders

编码器和解码器的权重通常是相关的转置,在某种程度上类似于主成分分析。
训练过程通常是一层一层训练，如通过RBM进行训练，然后通过反向传播计算整个网络。

#### Denoising autoencoders

去噪自动编码器将噪声添加到每个输入$x_n$，获得噪声版本$\tilde{x}_{n}$，然后训练机器，目的是从噪声版本$\tilde{x}_{n}$中恢复输入$x_n$。
可以通过最小化经验风险实现：
$$
\sum_{n=1}^{N} \ell\left(x_{n}, G_{\theta}\left(F_{\theta}\left(\tilde{x}_{n}\right)\right)\right)
$$

#### Probabilistic autoencoders

使用概率编码器和译码器而不是确定性的编码器和译码器。
将解码器视为变分分布,学习可以通过EM算法的一个变体，称为VAE变分自编码器。

### Ranking排序

当有可用的排序示例时，排序可以被公式化为监督学习问题。
为每个网络包括一个顶点并将相邻矩阵写为：
$$
...
$$
 PageRank将网络的排名计算为：
$$
 p_{i}=(1-d)+d \sum_{j \neq i} \frac{L_{i j}}{C_{j}} p_{j}
$$
页面的排名是通用排名等于 1 的加权总和，这使得可以选择新页面以及来自其他页面的聚合“投票”。
如果任何其他页面$j$链接到页面$i$ 并且它的投票等于它自己的排名除以传出链接的总数，即 $p_j/C_j$ ，则它会投票给页面$i$。
如果页面$i$被排名较高的页面链接，则页面$i$排名较高。可以递归地求解方程以获得所有页面的排名。
...
概率模型中的结构可以方便地以图形的形式表示。从根本上讲，概率模型中的结构属性相当于条件独立性假设。

### Bayesian Networks贝叶斯网络

#### Basic

BN 通过有向图对概率分解或等效地编码一组条件独立关系。起点是一组通用 $K \space rvs \{x_1, ..., x_K\}$ 概率的链式法则：
$$
\begin{aligned}
p\left(x_{1}, \ldots, x_{k}\right) & =p\left(x_{1}\right) p\left(x_{2} \mid x_{1}\right) \ldots p\left(x_{K} \mid x_{1}, \ldots, x_{K-1}\right) \\
& =\prod_{k=1}^{K} p\left(x_{k} \mid x_{1}, \ldots, x_{k-1}\right)
\end{aligned}
 $$
其中变量的顺序是任意的，不编码额外的结构信息。

条件独立性假设可以写为：
$$
\mathrm{x}_{w} \perp\left\{\mathrm{x}_{w^{\prime}}\right\}_{w^{\prime} \neq w} \mid \mathrm{t}
 $$
或者采用马尔科夫链表述：
$$
\mathrm{x}_{w}-\mathrm{t}-\left\{\mathrm{x}_{w^{\prime}}\right\}_{w^{\prime} \neq w}
 $$
将链式法则应用到该序列上：
$$
p(x, t)=p(t) \prod_{w=1}^{W} p\left(x_{w} \mid t\right)
 $$
联合分布分解为：
$$
p\left(x_{\mathcal{D}}, t_{\mathcal{D}}\right)=\prod_{n=1}^{N} p\left(t_{n}\right) \prod_{w=1}^{W} p\left(x_{w n} \mid t_{n}\right)
 $$

##### BN定义

BN 是一个有向无环图 (DAG)，其顶点表示 $rvs\space \{x_1, ...., x_K\}$ 以及相关联的联合分布，该分布分解为：
$$
p\left(x_{1}, \ldots, x_{K}\right)=\prod_{k=1}^{K} p\left(x_{k} \mid x_{\mathcal{P}(k)}\right)
 $$
BN 对局部条件独立关系进行编码：
$$
\mathrm{x}_{k} \perp\left\{\mathrm{x}_{1}, \ldots, \mathrm{x}_{k-1}\right\} \mid \mathrm{x}_{\mathcal{P}(k)}
 $$

##### When to use BNs

当可以识别变量之间的因果关系时，BN 是合适的模型。
在这种情况下，变量存在自然顺序，因此顺序中较晚出现的 rvs 是由前面变量的子集引起的。

##### Sampling from a BN

由BN编码的有序变量$\{x_1,..., x_K\}$之间的因果关系使得至少在原则上从BN中抽取样品变得容易。
方法：ancestral sampling祖先采样
生成$\mathrm{x}_{1} \sim p\left(x_{1}\right)$，$\mathrm{x}_{2} \sim p\left(x_{2} \mid x_{\mathcal{P}(2)}\right)$，按顺序依次生成。

##### Hidden Markov Models (HMMs)

HMM 编码**两个假设**：
1. 每个样本$z_i$仅通过前一个样本$z_{i−1}$依赖于过去的样本
2. 每个测量数据 $x_i$ 仅取决于 $z_i$

可以将联合分布写为：
$$
p(x, z)=p\left(z_{1}\right) p\left(x_{1} \mid z_{1}\right) \prod_{i=1}^{D} p\left(z_{i} \mid z_{i-1}\right) p\left(x_{i} \mid z_{i}\right)
 $$
...
全局独立假设：参数$\pi \text { and }\left\{\pi_{w \mid t}\right\}$是随机变量。
进行概率向量 π 的 Dirichlet 先验和参数$\left\{\pi_{w \mid t}\right\}$ 的 Beta 先验的标准假设，则每个联合概率分布因式分解为：
$$
\begin{aligned}
p\left(x, t, \pi, \pi_{w \mid t} \mid \alpha, a, b\right) & =\operatorname{Dir}(\pi \mid \alpha) \prod_{t=1}^{T} \prod_{w=1}^{W} \operatorname{Beta}\left(\pi_{w \mid t} \mid a, b\right) \\
& \times \operatorname{Cat}(t \mid \pi) \prod_{w=1}^{W} \operatorname{Bern}\left(x_{w} \mid \pi_{w \mid t}\right)
\end{aligned}
 $$

#### Global Conditional Independence全局独立假设

BN由编码在联合分布的因子化和支持DAG中的局部条件独立性属性定义。

BN还可用于评估类型的一般全局条件独立性查询：给定的变量子集A是否独立于另一个集合B是否以第三个变量子集C为条件？

##### D分离算法

1. 从原始DAG中建立子图$G'$，$G'$通过保留子集A，B和C中的所有顶点，以及通过从A，B和C的顶点向后移动所遇到的所有边缘和顶点。
2. 通过从$G'$中删除所有来自C的边缘生成$G''$。
3. 如果没有路径，忽略边缘的方向性，在A节点和B节点之间，则条件独立关系$A ⊥ B|C$保持。 如果存在这样的路径，那么至少有一个关节分布，该分布将该条件$A ⊥ B |C$不存在的给定DAG。

#### Learning 

由于概率因子对可学习参数$\mu$的依赖性，由 BN 编码的联合分布可以写为：
$$
p\left(x_{1}, \ldots, x_{K}\right)=\prod_{k=1}^{K} p\left(x_{k} \mid x_{\mathcal{P}(k)}, \mu_{k \mid x_{\mathcal{P}(k)}}\right)
 $$
通常情况，$p\left(x_{k} \mid x_{\mathcal{P}(k)}, \mu_{k \mid x_{\mathcal{P}(k)}}\right)$属于指数族或者GLM。

##### Fully Observed Data with Separate Parameters具有单独参数的完全观察数据

对数似然函数可以因式分解为：
$$
\begin{aligned}
\ln p(\mathcal{D} \mid \mu) & =\sum_{n=1}^{N} \sum_{k=1}^{K} \ln p\left(x_{k n} \mid x_{\mathcal{P}(k) n}, \mu_{k \mid x_{\mathcal{P}(k) n}}\right) \\
& =\sum_{k=1}^{K} \sum_{n=1}^{N} \ln p\left(x_{k n} \mid x_{\mathcal{P}(k) n}, \mu_{k \mid x_{\mathcal{P}(k) n}}\right) \\
& =\sum_{k=1}^{K} \sum_{x_{\mathcal{P}(k)}} \sum_{n \in \mathcal{N}_{x_{\mathcal{P}(k)}}} \ln p\left(x_{k n} \mid x_{\mathcal{P}(k)}, \mu_{k \mid x_{\mathcal{P}(k)}}\right)
\end{aligned}
 $$
定义一组索引：
$$
\mathcal{N}_{x_{\mathcal{P}(k)}}=\left\{n: x_{\mathcal{P}(k) n}=x_{\mathcal{P}(k)}\right\}
 $$
考虑建模为二元变量$\mathrm{x}_{k} \sim \operatorname{Bern}\left(\mu_{k \mid x_{\mathcal{P}(k)}}\right)$的例子，ML估计可以写为：
$$
\hat{\mu}_{k \mid x_{\mathcal{P}(k)}, M L}=\frac{\sum_{n \in \mathcal{N}_{x_{\mathcal{P}(k)}}} x_{k n}}{\left|\mathcal{N}_{x_{\mathcal{P}(k)}}\right|}
 $$
在参数的全局独立假设下， MAP 估计以类似的方式分解，这同样适用于贝叶斯方法。

##### Notes on the General Case

通过共享参数，获取ML和MAP估计需要在所有共享相同参数的变量中汇总统计数据。

在缺失数据的情况下，学习通常涉及第6章中描述的EM算法或者approximate learning counterparts近似学习对应器

### Markov Random Fields（MRF）马尔科夫随机场

#### Basic

MRF 通过无向图编码概率因式分解，或者等效地编码一组条件独立关系。

##### MRF定义

MRF 是一个无向图，其顶点表示具有关联联合分布的 rvs，该联合分布因式分解为：
$$
p(x)=\frac{1}{Z} \prod_{c} \psi_{c}\left(x_{c}\right)
 $$
其中配分函数
$$
Z=\sum_{x} \prod_{c} \psi_{c}\left(x_{c}\right)
 $$

##### 特点

1. $ψ_c(x_c)$的较大值对应于更可能发生的配置$x_c$。
2. 因子通常不是概率分布，即它们未归一化为求和或积分为一。
3. 每个因子都不区分条件反射和条件反射 rvs，集团$x_c$中的所有变量都可以在定义潜在$ψ_c(x_c)$的值方面发挥相同的作用。

##### When to use MRFs

MRF 适合于模拟变量之间的相容性或不相容性的相互关系，而不是因果关系。

##### Evaluating Probabilities and Sampling from an MRF评估概率并从MRF采样

MRF 的特征在某些应用中带来了潜在的建模优势，但同时也增加了评估联合概率分布和从联合分布中抽样的难度。

计算时需要计算配分函数$Z$，但当类别或者向量较多时是难以处理的。

MRF 不允许祖先采样，因为$x$中各个 rvs 的所有条件分布通常通过配分函数联系在一起。

##### MRF方法

将一个变量或者潜在变量与每个类别相关联会产生因式分解的联合分布：
$$
p(x, z)=\frac{1}{Z} \prod_{\{i, j\}} \psi_{i, j}\left(z_{i}, z_{j}\right) \cdot \prod_{i} \psi_{i}\left(z_{i}, x_{i}\right)
 $$
在Ising model伊辛模型中，变量是双极的，取值区间是$[-1, 1]$，势能定义分别为：$\psi_{i j}\left(z_{i}, z_{j} \mid \eta_{1}\right)=\exp \left(-E\left(z_{i}, z_{j} \mid \eta_{1}\right)\right)$，$\psi_{i}\left(z_{i}, x_{i} \mid \eta_{2}\right)=\exp \left(-E\left(z_{i}, x_{i} \mid \eta_{2}\right)\right)$，从而得到能量函数：
$$
E\left(z_{i}, z_{j} \mid \eta_{1}\right)=-\eta_{1} z_{i} z_{j} \text { and } E\left(z_{i}, x_{i} \mid \eta\right)=-\eta_{2} z_{i} x_{i}
 $$
1. 当$z_i$和$z_j$相等时，较大的自然参数$η_1 > 0$会产生较大的概率或较低的能量
2. 较大的$η_2 > 0$有利于$z_i = x_i$的配置，即具有低噪声。

#### Global Conditional Independence全局独立假设

MRF 以类似于 BN 的方式，能够在图中全局评估条件独立性属性。

##### Hammersley–Clifford定理

如果势能函数$ψ_c(x_c)$ 对所有类别$c$都是严格正的，则$A ⊥ B|C$可以简单验证，步骤如下：
1. 消除C中的所有变量和所有相连的边。
2. 如果A 和B 中的rvs 之间没有路径，则关系$A ⊥ B|C$成立；相反，如果存在一条路径，则存在至少一个联合分布，该分布对手头的无向图进行因式分解，其中关系$A ⊥ B|C$不成立。

#### Learning

分区函数使 MRF 中的学习变得复杂。
分区函数将所有参数耦合在一起。这使得即使在具有单独参数的完全观察情况下，也无法单独执行与每个团相关的参数$\eta_c$的估计。

具有基于能量的势的 MRF 要么属于指数族，当因子为对数正态时，要么属于更一般的基于能量的模型。都可以通过基于梯度的算法求解。

#### Converting BNs to MRFs

给定一个分解了的BN可以定义潜在函数：
$$
\psi_{k}\left(x_{k}, x_{\mathcal{P}(k)}\right)=p\left(x_{k} \mid x_{\mathcal{P}(k)}\right)
 $$
从而分解可以写为：
$$
p(x)=\prod_{k=1}^{K} \psi_{k}\left(x_{k}, x_{\mathcal{P}(k)}\right)
 $$
通过以下两步，可以直接从定义BN的DAG中得到对应的无向图：
1. 通过无向边连接所有对。
2. 使所有边无向。

### Bayesian Inference in Probabilistic Graphical Models概率图形模型中的贝叶斯推理

贝叶斯推理相当于计算给定观测变量的未观测变量或潜在变量的后验概率。

在执行贝叶斯推理时，要计算的数量时后验分布：
$$
p\left(z_{i} \mid x\right)=\frac{p\left(x, z_{i}\right)}{p(x)}
 $$
其中：
$$
\begin{aligned}
p\left(x, z_{i}\right) & =\sum_{z_{-i}} p(x, z) \\
p(x) & =\sum_{z_{i}} p\left(x, z_{i}\right)
\end{aligned}
 $$
复杂性在计算总和的相应潜变量的数量上指数化，因此它可能是令人难以接受的。

由概率图形模型编码的结构可以帮助降低所讨论的贝叶斯推理的复杂性。因此，概率图模型不仅可以通过控制模型的容量来增强学习，还可以实现贝叶斯推理。
考虑MRF定义的联合分布，边缘化需要解决和积推理任务：
$$
\sum_{z} \prod_{c} \psi_{c}\left(x_{c}\right)
 $$
计算MRF的配分函数Z的问题是和积推理任务的一个特例。

## 近似推理与学习

学习和推理任务通常因需要计算给定观察变量$x$的未观察变量$z$的后验分布$p(z|x)$而变得困难。这个任务需要计算归一化边际：
$$
p(x)=\sum_{z} p(x, z)
 $$
...
将归一化边际式子改写为集合平均值：
$$
p(x)=\sum_{z} p(z) p(x \mid z)=\mathrm{E}_{\mathrm{z} \sim p(z)}[p(x \mid \mathrm{z})]
 $$
MC方法背后的一般思想是用随机生成样本的经验平均值代替整体平均值。样本是从潜在变量的边际分布$p(z)$中生成的，然后通过经验平均来近似集合平均：
$$
p(x) \simeq \frac{1}{M} \sum_{m=1}^{M} p\left(x \mid \mathrm{z}_{m}\right)
 $$
根据大数定律，这个估计是一致的，因为当M很大时，它以概率1趋向于整体平均值。此外，近似尺度的误差为 $1/ \sqrt{(M)}$。

#### Importance Sampling重要性采样

最初的边际分布可以表示为$\operatorname{rv~} \mathrm{z} \sim q(z)$上的整体平均值：
$$
\begin{aligned}
p(x) & =\sum_{z} p(z) p(x \mid z) \frac{q(z)}{q(z)} \\
& =\sum_{z} q(z) \frac{p(z)}{q(z)} p(x \mid z) \\
& =\mathrm{E}_{\mathrm{z} \sim q(z)}\left[\frac{p(\mathrm{z})}{q(\mathrm{z})} p(x \mid \mathrm{z})\right]
\end{aligned}
 $$
**def：Importance Sampling**
生成$M$个i.i.d.样本$z_m ∼ q(z)$，$m = 1，...,M$，然后计算经验近似：
$$
p(x) \simeq \frac{1}{M} \sum_{m=1}^{M} \frac{p\left(\mathrm{z}_{m}\right)}{q\left(\mathrm{z}_{m}\right)} p\left(x \mid \mathrm{z}_{m}\right)
 $$

#### Markov Chain Monte Carlo (MCMC) via Gibbs Sampling

MCMC 方法不是从模拟 $p(z)$ 的分布中抽取样本来计算后验 $p(z|x) = p(x, z)/p(x)$ 的近似值，而是旨在获得样本 $\{z_m\}$直接来自后验 $p(z|x)$。

MCMC 方法从易于采样的马尔可夫链中生成一系列相关样本，Gibbs采样顺序采样随机变量的子集，对于每个子集，采样分布是通过对包括被采样的 rv 在内的所有因素的乘积进行归一化而获得的。

可以自动应用于任何典型的概率图模型。

### Variational Inference变分推理

VI方法在隐藏变量$z$上引入另外的分布，其被优化以接近所需的后验$p(z | x)$。

#### I-projection

优化问题的解可以表示为：
$$
\min _{q(z)} \mathrm{KL}(q(z) \| p(z \mid x))
 $$
根据第六章：
$$
\mathrm{KL}(q(z) \| p(x, z))=\mathrm{KL}(q(z) \| p(z \mid x))-\ln p(x)
 $$
从而优化问题等价于：
$$
\min _{q(z)} \mathrm{KL}(q(z) \| p(x, z))
 $$
其中：
$$
\mathrm{KL}(q(z) \| p(x, z))=-\mathrm{E}_{\mathrm{z} \sim q(z)}[\ln p(x, \mathrm{z})]-H(q)
 $$
因此，该问题可以等价于最小化变分自由能或者吉布斯自由能，或者负ELBO。

VI的关键思想是选择一个参数形式$q(z|ϕ)$作为变量后验来解决问题:
$$
\min _{\varphi} \mathrm{KL}(q(z \mid \varphi) \| p(x, z))
 $$
问题的解$q(z|ϕ^*)$被称为由给定参数化定义的分布集$\{q(z|ϕ)\}$中的分布$p(z|x)$的I-projection。

为了确保优化的可行性，通常选择参数化分布$q(z|ϕ)$以具有方便的因式分解和具有易于处理的分析形式的因子，例如指数族或GLM。

#### Amortized VI

定义一个推理变分分布$q(z|x, ϕ)$，它为$x = x$的任何值模拟$z$的后验分布。推理分布由向量$\phi$参数化，通常使用多层神经网络实现。在这种情况下，它通常被称为推理网络。这种方法称为摊销 VI。

关键优势：一旦学习了推理分布，就无需对之前未观察到的 x = $x$ 值执行 I 投影，可以直接将$q(z|x, ϕ)$应用于参数$ϕ$的学习值。

求解amortized I-projection问题：
$$
\min _{\varphi} \mathrm{E}_{\mathrm{x} \sim p(x)}[\operatorname{KL}(q(z \mid \mathrm{x}, \varphi) \| p(\mathrm{x}, z))]
 $$
#### M-projection

由于KL散度是不对称的，因此优化问题也可以写为：
$$
\min _{\varphi} \mathrm{KL}(p(z \mid x) \| q(z \mid \varphi))
 $$
该问题的解被称为M投影：
$$
q\left(z \mid \varphi^{*}\right)
 $$
如果$q\left(z \mid \varphi\right)$属于指数族：$q(z \mid \varphi) \propto \exp \left(\varphi^{T} u(z)\right)$，可以生成自然参数$\varphi^{*}$：
$$
\mathrm{E}_{\mathrm{z} \sim p(z \mid x)}[u(\mathrm{z})]=\mathrm{E}_{\mathrm{z} \sim q_{\left(z \mid \varphi^{*}\right)}}[u(\mathrm{z})]
 $$
##### 证明
KL散度可以写为：
$$
\mathrm{KL}(p(z) \| q(z \mid \varphi))=\ln Z(\varphi)-\varphi^{T} \mathrm{E}_{\mathrm{z} \sim p(z)}[u(\mathrm{z})]-H(p)
 $$
通用参数向量 $ϕ$ 和向量 $ϕ^∗$ 的 KL 散度之差可以写为：
$$
\begin{aligned}
& \mathrm{KL}(p(z) \| q(z \mid \varphi))-\mathrm{KL}\left(p(z) \| q\left(z \mid \varphi^{*}\right)\right) \\
= & \ln \left(\frac{Z(\varphi)}{Z\left(\varphi^{*}\right)}\right)-\left(\varphi-\varphi^{*}\right)^{T} \mathrm{E}_{\mathrm{z} \sim p(z)}[u(\mathrm{z})] \\
= & \mathrm{E}_{\mathrm{z} \sim q\left(z \mid \varphi^{*}\right)}\left[\ln \left(\frac{q\left(\mathrm{z} \mid \varphi^{*}\right)}{q(\mathrm{z} \mid \varphi)}\right)\right] \\
= & \mathrm{KL}\left(q\left(z \mid \varphi^{*}\right) \| q(z \mid \varphi)\right) .
\end{aligned}
 $$
由于后一个不等式是非负的并且当 $ϕ = ϕ^*$ 时等于零，因此自然参数的这种选择最小化了 KL 散度。

#### I-projection and M-projection

##### I-projection

1. I-projection 倾向于模式寻求和排他性，I投影往往会低估分布的方差。
2. I 投影通常在 $p(z|x)$ 较大的地方更准确

##### M-projection

M-projection 倾向于包含并跨越$p(z|x)$的整个支持。这是因为 M 投影倾向于避免 $q(z|ϕ)$ 在 $z$ 值处的零值，使得$p(z \mid x) \neq 0$以避免无限的 KL 散度。
...
1. 当$\alpha->0$时，$D_{\alpha}(p \| q)=\mathrm{KL}(q \| p)$；
2. 当$\alpha->1$时，$D_{\alpha}(p \| q)=\mathrm{KL}(p \| q)$
3. 当$\alpha \neq 0$时，α散度的驻点$\min _{\varphi} D_{\alpha}(p(x \mid z) \| q(z \mid \varphi))$与KL散度的驻点 $\min _{\varphi} \mathrm{KL}\left(p(x \mid z) \| p(x \mid z)^{\alpha} q(z \mid \varphi)^{1-\alpha}\right)$重合。

#### Mean Field Variational Inference（MFVI）平均场变异推理

MFVI是一种通用的、或黑盒子的Bayesian推理技术，例如Gibbs采样MC技术。
MFVI假设分解$q(z)=\prod_{j} q\left(z_{j}\right)$，一次对一个隐藏的变量$z_i$进行迭代的I投影，同时将因子$q_j(z_j)$修复为其他潜伏变量$z_j$与$j \neq i$。
对于每个因子$q_i(z_i)$，I 投影问题最小化变分自由能：
$$
-\mathrm{E}_{\mathrm{z}_{i} \sim q_{i}\left(z_{i}\right)}\left[\mathrm{E}_{j \neq i}[\ln p(x, \mathrm{z})]\right]-\sum_{j} H\left(q_{j}\left(z_{j}\right)\right)
 $$
定义期望：
$$
\mathrm{E}_{j \neq i}[\ln p(x, \mathrm{z})]=\mathrm{E}_{\left\{\mathrm{z}_{j}\right\}_{j \neq i} \sim \prod_{j \neq i} q_{j}\left(z_{j}\right)}\left[\ln p\left(x, z_{i},\left\{\mathrm{z}_{j}\right\}_{j \neq i}\right)\right]
 $$
忽略与$q_i(z_i)$无关的常数，等价于最小化:
$$
\min _{q_{i}} \mathrm{KL}\left(q_{i}\left(z_{i}\right) \| \exp \left(\mathrm{E}_{j \neq i}[\ln p(x, \mathrm{z})]\right)\right)
 $$
通过将散度的右侧参数归一化，得到该问题解决方法：
$$
q_{i}\left(z_{i}\right)=\frac{\exp \left(\mathrm{E}_{j \neq i}[\ln p(x, \mathrm{z})]\right)}{\sum_{z_{i}} \exp \left(\mathrm{E}_{j \neq i}[\ln p(x, \mathrm{z})]\right)}
 $$
进行循环对所有因子进行计算。

MFVI 迭代保证收敛到 KL 最小化问题的固定点。
评估期望的方法：
$$
\begin{aligned}
q_{i}\left(z_{i}\right) & \propto \exp \left(\mathrm{E}_{j \neq i}\left[\sum_{c} \ln \psi_{c}\left(x_{c}, \mathrm{z}_{c}\right)\right]\right) \\
& =\exp \left(\mathrm{E}_{j \neq i}\left[\sum_{c: z_{i} \in z_{c}} \ln \psi_{c}\left(x_{c}, \mathrm{z}_{c}\right)\right]\right)
\end{aligned}
 $$
考虑Ising Model，近似后验给出为：
$$
\begin{aligned}
q_{i}\left(z_{i}\right) & \propto \exp \left(\eta_{1} \sum_{\{i, j\}} \mathrm{E}_{\mathrm{z}_{j} \sim q\left(z_{j}\right)}\left[z_{i} \mathrm{z}_{j}\right]+\eta_{2} x_{i} z_{i}\right) \\
& =\exp \left(z_{i}\left(\eta_{1} \sum_{\{i, j\}} \mathrm{E}_{\mathrm{z}_{j} \sim q\left(z_{j}\right)}\left[\mathrm{z}_{j}\right]+\eta_{2} x_{i}\right)\right)
\end{aligned}
 $$
正则化：
$$
\begin{aligned}
q_{i}\left(z_{i}\right. & =1)=\frac{1}{1+\exp \left(-2\left(\eta_{1} \sum_{\{i, j\}} \mu_{j}+\eta_{2} x_{i}\right)\right)} \\
& =\sigma\left(2\left(\eta_{1} \sum_{\{i, j\}} \mu_{j}+\eta_{2} x_{i}\right)\right)
\end{aligned}
 $$
其中：$\mu_{i}=q_{i}\left(z_{i}=1\right)-q_{i}\left(z_{i}=-1\right)=2 q_{i}\left(z_{i}=1\right)-1$

#### Beyond MFVI

Bethe approach（LBP算法）：基于与联合分布$p(x, z)$相同的因式分解的方法，也可以解释为消息传递信念传播的应用。
Approximate Message Passing（AMP算法）：==loopy belief propagation==的简化

### Monte Carlo-Based Variational Inference基于蒙特卡罗的变分推理

基于 MC 的 VI 的关键思想是通过 MC 通过绘制一个或多个样本 $z_m ∼q(z)$来近似 KL 散度，其中 $m = 1, ...,M$，并计算经验平均值：
$$
\mathrm{KL}(q(z) \| p(x, z)) \simeq \frac{1}{M} \sum_{m=1}^{M}\left(\ln q\left(z_{m}\right)-\ln p\left(x, z_{m}\right)\right)
 $$
为了优化变分后验 $q(z|ϕ)$ 的参数 $ϕ$，实际上逼近 KL 散度的梯度更有用，如下所述。

#### REINFORCE approach

假设参数化后验变分可以计算梯度$\nabla_{\varphi} \ln q(z \mid \varphi)$，一个基于 SGD 的方案如下。

I投影问题中关于变分参数$\phi$的KL散度梯度可以写为：
$$
\nabla_{\varphi} \mathrm{KL}(q(z \mid \varphi) \| p(x, z))=\mathrm{E}_{\mathrm{z} \sim q(z \mid \varphi)}\left[\nabla_{\varphi} \ln q(\mathrm{z} \mid \varphi) l_{\varphi}(x, \mathrm{z})\right]
 $$
定义学习信号：
$$
l_{\varphi}(x, z)=\ln q(z \mid \varphi)-\ln p(x, z)
 $$
使用以下两个等式：
$$
\nabla_{\varphi} \ln q(z \mid \varphi)=\nabla_{\varphi} q(z \mid \varphi) / q(z \mid \varphi)
 $$
$$
\mathrm{E}_{\mathrm{z} \sim q(z \mid \varphi)}\left[\nabla_{\varphi} \ln q(\mathrm{z} \mid \varphi)\right]=0
 $$
基于 MC 的 VI 方法通过绘制一个或多个样本$z_m ∼q(z|ϕ)$来评估给定值$ϕ$下梯度的 MC 近似值，其中$m = 1, ...,M$，然后计算：
$$
\nabla_{\varphi} \mathrm{KL}(q(z \mid \varphi) \| p(x, z)) \simeq \frac{1}{M} \sum_{m=1}^{M}\left[\nabla_{\varphi} \ln q\left(z_{m} \mid \varphi\right) l_{\varphi}\left(x, z_{m}\right)\right]
 $$
该方法也可称为似然比。

**缺点**：可能出现高方差问题。

#### Reparametrization trick重新参数化技巧

利用KL散度公式变形重写I投影公式：
$$
\mathrm{KL}(q(z \mid \varphi) \| p(x, z))=-\mathrm{E}_{\mathrm{w} \sim s(z)}\left[\ln p\left(x \mid G_{\varphi}(\mathrm{w})\right)\right]+\mathrm{KL}(q(z \mid \varphi) \| p(z))
 $$
可以通过如下方程估计梯度：
$$
\nabla_{\varphi} \mathrm{KL}(q(z \mid \varphi) \| p(x, z)) \simeq-\frac{1}{M} \sum_{m=1}^{M}\left[\nabla_{\varphi} \ln p\left(x \mid G_{\varphi}\left(w_{m}\right)\right)\right]+\nabla_{\varphi} \mathrm{KL}(q(z \mid \varphi) \| p(z))
 $$
这种方法倾向于提供比 REINFORCE 梯度方差更低的估计，因为它利用了分布$q(z|ϕ)$的结构。

### Approximate Learning近似学习

存在隐藏变量时的 ML 学习可以通过最大化 ELBO 来近似，此时ELBO算法可以改写为：
$$
\sum_{n=1}^{N} \ln p\left(x_{n} \mid \theta\right)-\operatorname{KL}\left(q\left(z \mid x_{n}, \varphi\right) \| p\left(z \mid x_{n}, \theta\right)\right)
 $$
对于任何固定的$ϕ$，在$θ$上优化 ELBO 会在变分正则化项下最大化第一项中的似然函数，该项惩罚与所选变分后验$q(z| x, θ)$显着不同的后验$p(z| x, φ)$。
因此，为变分后验选择给定模型可以驱动学习，并且应该被视为模型或超参数选择 。

ELBO 在模型和变分参数上的最大化可以以不同的方式进行。
1. 可以通过近似推理执行 E 步骤来使用 EM 来评估潜在变量的后验。当 VI 用于此目的时，生成的方案称为变分 EM。
2. 通过利用 REINFORCE 方法或重新参数化技巧，对参数向量$θ$和$ϕ$使用 SGD。

重新参数化技巧方法例如用于生成建模的 VAE 方法，也称为深度高斯潜在模型，以及的黑盒学习方法。 KL 散度也可以用 GAN 方法中的对抗性学习散度代替。

当使用 M 投影而不是 ELBO 最大化产生的 I 投影更新变分参数时，联合优化模型和变分参数的方法产生唤醒-睡眠算法。

# Conclusions

## Concluding Remarks结束语

- Privacy：在许多应用中，用于训练机器学习算法的数据集包含敏感的私人信息。 因此，必须确保所学模型不泄露关于培训单元的任何信息。 这一制约因素可以使用差别隐私的概念来制定。 保障个别数据点隐私的典型技术包括在通过SGD进行培训时为梯度增加噪音，并依赖于经过不同数据子集培训的专家的混合。
- Robustness：各种机器学习模型对数据的小变化敏感，在轻微、正确选择的情况下产生错误响应，解释变量的变化。仅仅基于黑盒观察，修改培训程序，以确保对抗性实例的可靠性，是一个具有重要实际影响的研究领域。
- Computing platforms and programming frameworks：为了扩大机器学习应用的规模，有必要利用分布式计算架构和相关的标准编程框架。利用基于退火的量子计算机的能力作为采样器或离散优化器
- Transfer learning：为特定任务训练的机器目前需要重新训练，以便重新用于不同的任务。迁移学习包括不同的相关范式，例如多任务学习、终身学习、zeroshot 学习和领域适应。
	- 在多任务学习中，多个任务是同时学习的。基于神经网络的多任务学习的典型解决方案规定了为不同任务训练的神经网络之间存在公共隐藏层。
	- 终身学习通过利用在先前训练阶段积累的知识来更新接受过多项任务训练的机器，以执行新任务。
	- 零样本学习是指能够识别不可见类的模型，其训练示例仅适用于相关但不同的类。这通常需要学习类表示的任务，例如原型向量，它通过固定的概率机制在类中生成数据。
- Domain adaptation：在许多学习问题中，可用数据与测试算法的数据有不同的分布。一个泛化的PAC理论分析域适应，获得在所需的测试分布下的泛化误差的界限，作为训练和测试分布之间的差异的函数。
- Communication-efficient learning：在分布式计算平台中，数据通常在处理器之间进行划分，而处理器之间的通信需要延迟和能耗。一个重要的研究问题是如何在学习成绩和交流开销之间进行最佳权衡。
- Reinforcement learning：在强化学习中，人们希望学习观察到的世界状态$x$和动作$t$之间的最优映射，比如$q(t|x, θ)$。与监督学习不同，最佳动作$t$是未知的，但机器会根据动作的效果观察奖励/惩罚信号。一种称为深度强化学习的流行方法使用深度神经网络对映射$q(t|x, θ)$进行建模。通过使用 REINFORCE 方法估计梯度，通过 SGD 将其训练为最大化平均奖励。