...
只关注$w$时，优化器为：$$\min _{w} L_{\mathcal{D}}(w)=\frac{1}{N} \sum_{n=1}^{N}\left(\mu\left(x_{n}, w\right)-t_{n}\right)^{2}$$，优化目标是$L_{\mathcal{D}}(w)$，即训练损失。当$\hat{t}(x)=\mu(x, w)$时，与[ERM]算法一致。
##### ERM算法求解
ERM可以用封闭形式求解：
...
在$q(x)$上最小化，得到极小极大冗余：$$\begin{aligned}
